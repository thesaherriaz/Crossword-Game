...
class CrosswordGame:
    ...
    def place_word(self):
        ...
        if word in self.used_words:
            self.status_label.config(text=f"'{word}' has already been used.")
            return
        
        if word not in self.game.words_with_positions:
            self.status_label.config(text=f"'{word}' is not a valid word.")
            return

        # Retrieve the word's predefined position and direction
        (start_row, start_col), direction = self.game.words_with_positions[word]

        # Place the word on the grid
        if direction == "right":
            for i, char in enumerate(word):
                label = self.grid_labels[start_row][start_col + i]
                if label:  # Ensure the label exists
                    label.config(text=char, bg="green")
        elif direction == "down":
            for i, char in enumerate(word):
                label = self.grid_labels[start_row + i][start_col]
                if label:  # Ensure the label exists
                    label.config(text=char, bg="green")
                    print(start_row, start_col)

        # Update scores and switch players
    ...
    def update_grid_with_word(self, word, index, start, is_row):
        if is_row:
            for i, char in enumerate(word):
